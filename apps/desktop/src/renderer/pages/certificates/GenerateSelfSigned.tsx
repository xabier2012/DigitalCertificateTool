...
export default function GenerateSelfSigned() {
  ...
  const [ou, setOu] = useState('Development');
  ...
  const [certFileName, setCertFileName] = useState('certificate.pem');

  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [successDialogOpen, setSuccessDialogOpen] = useState(false);
  const [resultPaths, setResultPaths] = useState<{ keyPath: string; certPath: string } | null>(null);
  ...
  return (
    <Box>
      ...
      <Typography variant="body1" color="text.secondary" sx={{ mb: 1 }}>
        Crea un certificado autofirmado para desarrollo y pruebas
      </Typography>

      <Alert severity="warning" icon={<WarningIcon />} sx={{ mb: 3 }}>
        <strong>Atención:</strong> Los certificados autofirmados no serán confiables por defecto en navegadores y sistemas.
        Solo úsalos para desarrollo local y pruebas. Para producción, usa un CSR y una CA de confianza.
      </Alert>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={6}>
            <TextField
              ...
              placeholder="localhost o Mi Certificado Dev"
              helperText="Nombre de dominio o identificador"
            />
          </Grid>
          ...
          <Grid item xs={12}>
            ...
            <Box sx={{ display: 'flex', gap: 1, mb: 2 }}>
              ...
              <TextField
                ...
                placeholder={newSanType === 'DNS' ? 'localhost' : '127.0.0.1'}
                sx={{ flex: 1 }}
              />
            ...
            <Collapse in={showAdvanced}>
              <Grid container spacing={2} sx={{ mt: 1 }}>
                ...
                <Grid item xs={12} md={6}>
                  <TextField
                    fullWidth
                    label="Nombre archivo certificado"
                    value={certFileName}
      ...
      <Dialog open={successDialogOpen} onClose={() => setSuccessDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          ...
          <Box sx={{ bgcolor: 'grey.100', p: 2, borderRadius: 1, mt: 2 }}>
            ...
            <Typography sx={{ fontFamily: 'monospace' }}>{resultPaths?.certPath}</Typography>
          </Box>

          <Alert severity="warning" sx={{ mt: 2 }}>
            Recuerda: Este certificado no será confiable por defecto. Solo úsalo para desarrollo.
          </Alert>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => window.electronAPI.shell.showItemInFolder(resultPaths?.certPath || '')}>
            Abrir carpeta
          </Button>
          <Button onClick={handleReset}>Nuevo certificado</Button>
